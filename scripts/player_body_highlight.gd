class_name PlayerBodyHighlight
extends Node2D
...
func _has_damaged_fatal_organ(part_name: String) -> bool:
	if player_ref == null:
		return false
	var bp = player_ref.body_parts.get(part_name)
	if bp == null:
		return false
...
func _input(event: InputEvent):
	if _tooltip != null and event is InputEventMouseMotion:
	...
	for part_name in parts:
		if parts[part_name]["rect"].has_point(local_pos):
...
func _draw():
	...
	for part_name in parts:
		...
		elif player_ref.body_parts.has(part_name):
			var bp = player_ref.body_parts[part_name]
			match bp.status:
